...
async def update_admin(client, message):
    global a
    admins = await client.get_chat_members(message.chat.id, filter="Yöneticiler")
    new_ads = []
    for u in admins:
...
async def skip(_, message: Message):
    global que
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❗ Atlamak İçin Hiçbir Şey Oynatılmıyor!")
    else:
...
async def admincache(client, message: Message):
    ...
    #await message.reply_text("♪ EzelMusicBot ♪=❇️ Admin Listesi Yenilendi!")
